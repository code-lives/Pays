...
class Byte
{
    ...
    {
        if (!isset($config['app_id']) || empty($config['app_id'])) throw new \Exception('not empty app_id');
        if (!isset($config['secret']) || empty($config['secret'])) throw new \Exception('not empty secret');
        if (!isset($config['merchant_id']) || empty($config['merchant_id'])) throw new \Exception('not empty merchant_id');
        if (!isset($config['salt']) || empty($config['salt'])) throw new \Exception('not empty salt');
        if (!isset($config['notify_url']) || empty($config['notify_url'])) throw new \Exception('not empty notify_url');
        if (!isset($config['token']) || empty($config['token'])) throw new \Exception('not empty notify_url');
        $class = new self();
